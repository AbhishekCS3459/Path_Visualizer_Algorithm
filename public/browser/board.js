...
Board.prototype.drawShortestPath = function (
  ...
  if (this.currentAlgorithm !== "bidirectional") {
    ...
    if (this.middleNode !== this.target && this.middleNode !== this.start) {
      ...
      secondCurrentNode =
        this.nodes[this.nodes[this.middleNode].otherpreviousNode];
...
Board.prototype.changeStartNodeImages = function () {
  ...
  if (unweighted.includes(this.currentAlgorithm)) {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      document.styleSheets["1"].rules[j].style.backgroundImage =
        backgroundImage.replace("triangle", "spaceship");
    }
  } else {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      let backgroundImage =
        document.styleSheets["1"].rules[j].style.backgroundImage;
      document.styleSheets["1"].rules[j].style.backgroundImage =
        backgroundImage.replace("spaceship", "triangle");
    }
  }
  if (this.currentAlgorithm === "bidirectional") {
    document.getElementById(
...
Board.prototype.toggleTutorialButtons = function () {
  ...
  function nextPreviousClick() {
    if (counter === 1) {
      document.getElementById("tutorial").innerHTML = `<h3>Welcome to the Pathfinding Visualizer!</h3><h6>This brief tutorial will guide you through the various features of this application.</h6><p>If you're eager to get started, feel free to click the "Skip Tutorial" button below. Otherwise, click "Next"!</p><div id="tutorialCounter">1/9</div><img id="mainTutorialImage" src="public/styling/c_icon.png"><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 2) {
      document.getElementById("tutorial").innerHTML = `<h3>Exploring the Pathfinding Algorithm</h3><h6>At its core, a pathfinding algorithm aims to find the shortest path between two points. This application provides a visual representation of various pathfinding algorithms in action and more!</h6><p>All algorithms in this application are optimized for a 2D grid, where 90-degree turns have a "cost" of 1 and movements from one node to another have a "cost" of 1.</p><div id="tutorialCounter">${counter}/9</div><img id="mainTutorialImage" src="public/styling/path.png"><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 3) {
      document.getElementById("tutorial").innerHTML = `<h3>Selecting an Algorithm</h3><h6>Choose an algorithm from the drop-down menu labeled "Algorithms".</h6><p>It's important to note that some algorithms are <i><b>unweighted</b></i>, while others are <i><b>weighted</b></i>. Unweighted algorithms do not consider turns or node weights, whereas weighted algorithms do. Additionally, not all algorithms guarantee the shortest path.</p><img id="secondTutorialImage" src="public/styling/algorithms.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 4) {
      document.getElementById("tutorial").innerHTML = `<h3>Meet the Algorithms</h3><h6>Not all algorithms are created equal.</h6><ul><li><b>Dijkstra's Algorithm</b> (weighted): the father of pathfinding algorithms; guarantees the shortest path</li><li><b>A* Search</b> (weighted): arguably the best pathfinding algorithm; uses heuristics to find the shortest path faster than Dijkstra's Algorithm</li><li><b>Greedy Best-first Search</b> (weighted): a faster, more heuristic-heavy version of A*; does not guarantee the shortest path</li><li><b>Swarm Algorithm</b> (weighted): a combination of Dijkstra's Algorithm and A*; does not guarantee the shortest path</li><li><b>Convergent Swarm Algorithm</b> (weighted): a faster, more heuristic-heavy version of Swarm; does not guarantee the shortest path</li><li><b>Bidirectional Swarm Algorithm</b> (weighted): Swarm from both ends; does not guarantee the shortest path</li><li><b>Breath-first Search</b> (unweighted): a great algorithm; guarantees the shortest path</li><li><b>Depth-first Search</b> (unweighted): not ideal for pathfinding; does not guarantee the shortest path</li></ul><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 5) {
      document.getElementById("tutorial").innerHTML = `<h3>Adding Walls and Weights</h3><h6>Click on the grid to add walls. Press the "W" key while clicking to add weights. Generate mazes and patterns from the drop-down menu labeled "Mazes & Patterns".</h6><p>Walls are impenetrable, meaning that a path cannot pass through them. Weights, on the other hand, do not block the path but increase its "cost" for traversal. In this application, moving through a weight node has a "cost" of 15.</p><img id="secondTutorialImage" src="public/styling/walls.gif"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 6) {
      document.getElementById("tutorial").innerHTML = `<h3>Adding a Bomb</h3><h6>Click the "Add Bomb" button.</h6><p>Adding a bomb alters the behavior of the chosen algorithm. The algorithm will first attempt to find and defuse the bomb before seeking the target node. Note that the Bidirectional Swarm Algorithm does not support the addition of a bomb.</p><img id="secondTutorialImage" src="public/styling/bomb.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 7) {
      document.getElementById("tutorial").innerHTML = `<h3>Dragging Nodes</h3><h6>Click and drag the start 
      node, bomb node, and target node to reposition them.</h6><p>Note that you can drag nodes even 
      after an algorithm has finished running. This allows you to instantly explore different paths.</p><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 8) {
      document.getElementById("tutorial").innerHTML = `<h3>Visualization and More</h3><h6>Use the buttons in the navigation bar to visualize algorithms and perform other actions!</h6><p>You can clear the current path, clear walls and weights, clear the entire board, and adjust the visualization speed, all from the navigation bar. To access this tutorial again, click on "Pathfinding Visualizer" in the top-left corner of the screen.</p><img id="secondTutorialImage" src="public/styling/navbar.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
    } else if (counter === 9) {
      document.getElementById("tutorial").innerHTML = `<h3>Enjoy!</h3><h6>I hope you have as much fun exploring this visualization tool as I had building it!</h6><p>If you would like to see the source code for this application, check out my <a href="https://github.com/AbhishekCS3459/Detailed_PathFinding_Visualizer">GitHub repository</a>.</p><div id="tutorialCounter">${counter}/9</div><button id="finishButton" class="btn btn-default navbar-btn" type="button">Finish</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`;
